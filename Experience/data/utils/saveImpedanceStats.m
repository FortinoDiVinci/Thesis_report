function saveImpedanceStats(cycles, exp_list, path)
...
for id = 1:size(impedance_vect,1)
    for i = 1:3
        ...
        lw(i,1) = imp_data_sort(find(q(1) - 1.5*iqe <= imp_data_sort, 1, 'first'));  %q(1) - 1.5*iqe;
...
for id = 1:size(impedance_vect,1)
    for i = 1:3
        for j = 1:3
            ...
            lw(k,1) = imp_data_sort(find(q(1) - 1.5*iqe <= imp_data_sort, 1, 'first'));
...
for id = 1:size(impedance_vect,1)
    for i = 1:3
        ...
        lq(i,1) = q(1);
        uw(i,1) = imp_data_sort(find(q(3) + 1.5*iqe >= imp_data_sort, 1, 'last'));        
        lw(i,1) = imp_data_sort(find(q(1) - 1.5*iqe <= imp_data_sort, 1, 'first'));
