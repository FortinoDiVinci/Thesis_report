...
{
MessageAggregator::MessageAggregator(const std::string& world_frame_id, const std::string& arm_frame_id,
...
void MessageAggregator::onRealPaddlePose(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
  real_paddle_pose = msg;
}

void MessageAggregator::onVirtualPaddlePose(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
  virtual_paddle_pose = msg;
}

void MessageAggregator::onBallPose(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
  ball_pose = msg;
}

void MessageAggregator::onForceFeedback(const geometry_msgs::WrenchStamped::ConstPtr& msg)
{
  force_feedback = msg;
}

void MessageAggregator::onArmJointStates(const sensor_msgs::JointState::ConstPtr& msg)
{
  arm_joint_states = msg;
}

void MessageAggregator::logFrameHeightField(LogEntry& entry, const ros::Time& stamp, const std::string& frame_id)
{
  ...
  {
    ROS_WARN_THROTTLE(2.0, "cannot compute %s coordinates: %s", frame_id.c_str(), e.what());
