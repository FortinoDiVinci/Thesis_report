\begin{figure} [h]
	...
	\begin{minipage}[b]{.45\linewidth}
		\centering
		\subcaptionbox{Modèle articulaire dynamique (\cref{eq_hogan_cocontraction}) \label{sugfig_dynamic_model_hogan_1984}}
		{
			\begin{tikzpicture}			
			\coordinate (elbow) at (0,0);
			\coordinate (shoulder) at ($(elbow) + (0, 3.25)$);
	...
	\caption{Modélisation simpliste de l'articulation du coude, avec l'action agoniste du biceps $b$ et antagoniste du triceps $t$. Les muscles sont considérés comme de simple raideur variable, l'inertie du bras, la viscosité des muscles ainsi que le couple gravitationnel sont pris en compte lors des calculs présentés par \cite{hogan_adaptive_1984}. Ce modèle est valide pour de faibles vitesses (linéarisation de la viscosité).}
