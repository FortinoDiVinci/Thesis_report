\begin{figure} [h]
	...
	\begin{tikzpicture} [node distance = 2cm]	
		...
		\draw[{Latex[length=3mm]}-] (ce1.west) --+ (-3,0)
			node[align = center, above] {$\vect{x}_1$};
		\draw[{Latex[length=3mm]}-] (ce2.west) --+ (-3,0)
			node[align = center, below] {$\vect{x}_2$};	
	...
	\end{tikzpicture}
	\caption{Modèle à plusieurs entrées et sorties proposé par \textcite{perreault_multiple-input_1999}, avec $h_{ij}$ la fonction de transfert liant l'entrée $i$ à la sortie $j$.}
