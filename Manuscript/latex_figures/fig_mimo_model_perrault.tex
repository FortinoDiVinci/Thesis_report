\begin{figure} [h]
	...
	\begin{tikzpicture} [node distance = 2cm]	
		...
		\node[block, below of = h11, node distance = 1.5cm] (h12) {$h_{12}(t)$};	
		\node[block, below of = h12, node distance = 1.5cm] (h21) {$h_{21}(t)$};	
		...
		\draw[-{Latex[length=3mm]}] (cs1.east) --+ (1,0)
			node[align = center, below, midway] {$\vect{x}$};
		\draw[-{Latex[length=3mm]}] (cs2.east) --+ (1,0)
			node[align = center, above, midway] {$\vect{y}$};
			
		\draw[{Latex[length=3mm]}-] (ce1.west) --+ (-3,0)
			node[align = center, above] {$\vect{f}_x$};
		\draw[{Latex[length=3mm]}-] (ce2.west) --+ (-3,0)
			node[align = center, below] {$\vect{f}_y$};	
	...
	\end{tikzpicture}
	\caption{Modèle à plusieurs entrées et sorties proposés par \textcite{perreault_multiple-input_1999}.}
