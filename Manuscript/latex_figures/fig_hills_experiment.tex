\begin{figure} [h]
	...
	\begin{minipage}[b]{.6\linewidth}
		\centering
		\subcaptionbox{Expérience de Hill sur un muscle de grenouille tétanisé\label{fig_experience_hill_1938_i}}
		{
			\tikzsetnextfilename{fig_hills_experiment_a}
			\begin{tikzpicture}
				\node[inner sep=0pt] (schema) at (0,0)
				{\includegraphics[width=0.9\textwidth]{misc/fig_experience_hill}};
				\node (muscle) at ($ (schema.center) + (1.8,-.7) $) {(i)};
				\node (muscle) at ($ (schema.center) + (0.55,-.45) $) {(ii)};
				\node (muscle) at ($ (schema.center) + (3.15,0.9) $) {(iii)};
				\node (muscle) at ($ (schema.center) + (-2.2,-1.15) $) {(iv)};
			\end{tikzpicture}
		}		
	\end{minipage}
	\begin{minipage}[b]{.4\linewidth}
		\centering
		\subcaptionbox{Observations de l'expérience de Hill\label{fig_experience_hill_1938_ii}}
		{
			\tikzsetnextfilename{fig_hills_experiment_b}
			\begin{tikzpicture}
			\def\varXi{0.9}%
			\def\varW{2*\pi}%
			...
			\begin{axis}[
				...
				xtick={5},
				xlabel={time},
				x label style={at={(axis description cs:0.9,0)},anchor=north},
				ylabel={longueur},
	...
	\caption[Schéma de l'expérience conduite par \textcite{hill_heat_1938}]{ (\subref{fig_experience_hill_1938_i}) Expérience réalisée par Hill sur le gastrocnemius d'une grenouille (i), avec le stimulateur (ii) permettant de tétaniser le muscle, le mécanisme (iii) maintenant le muscle dans une configuration isométrique et la charge (iv). Ce schéma est une reproduction issue de \textcite[Chapitre~1]{mcmahon_muscles_1984}. (\subref{fig_experience_hill_1938_ii}) Jusqu'à $t_r$, l'expérience est dans la configuration dépeinte à gauche, la tension exercée par le muscle est $T$. Puis, le mécanisme libère la barre, le muscle se contracte alors rapidement de $\Delta x_1$, et la tension chute à une valeur de $T_0$ ce qui correspond à un comportement en raideur non amortie $K_{SEE}$, puis on observe une contraction graduelle en l'absence de changement de tension, ce qui est équivalent à une raideur $K_{PEE}$ amortie.}
