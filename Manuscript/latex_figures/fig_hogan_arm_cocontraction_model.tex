\begin{figure} [h]
	...
	\centering
	\subcaptionbox{Articulation du coude, avec biceps et triceps \parencite{hogan_adaptive_1984} \label{sugfig_hogan_1984}}
	{
		\tikzsetnextfilename{fig_hogan_arm_cocontraction_model_a}
		\begin{tikzpicture}
	...
	\begin{minipage}[b]{.45\linewidth}
		...
		{
			\tikzsetnextfilename{fig_hogan_arm_cocontraction_model_b}
			\begin{tikzpicture}			
			\coordinate (elbow) at (0,0);
			\coordinate (shoulder) at ($(elbow) + (0, 3.25)$);
			%
			...
			\node[above, xshift=3mm, yshift=2mm] at (S){$R$};
			...
			\node[above, xshift=3mm, yshift=2mm] at (S2){$D$};
			% from B the circle starts at 120 degree, let's se e the landing angle and use a spline
	...
	\caption[Modélisation simplifiée de l'articulation du coude \parencite{hogan_adaptive_1984}]{Modélisation simplifiée de l'articulation du coude, avec l'action agoniste du biceps $b$ et antagoniste du triceps $t$. Les muscles sont considérés comme de simples raideurs variables, l'inertie du bras, la viscosité des muscles ainsi que le couple gravitationnel sont pris en compte lors des calculs présentés par \textcite{hogan_adaptive_1984}. Ce modèle est valide pour de faibles vitesses (linéarisation de la viscosité), et de petites variations de posture (la dépendance articulaire \parencite{rack_effects_1969, vredenbregt_surface_1973} étant négligée).}
