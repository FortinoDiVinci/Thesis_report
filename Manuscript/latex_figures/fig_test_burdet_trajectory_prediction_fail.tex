% figure 
\begin{figure}[!htb] 
	\tikzsetnextfilename{fig_test_burdet_trajectory_prediction_fail}
	\centering
		\resizebox{\textwidth}{!}{
			\begin{tikzpicture}
			\centering
...
%				\coordinate (c2) at (axis cs:580, \pgfkeysvalueof{/pgfplots/ymax});				
	...
	\caption{Exemple d'application de la méthodologie de \textcite{burdet_method_2000} pour une estimation de trajectoire en position sur \SI{200}{\milli\second} peu satisfaisante (cycle 25 de la \cref{fig_unp_position_test_trajectory}). A la fin de cette fenêtre temporelle, l'erreur est de \SI{5.9}{\milli\meter} pour une amplitude de mouvement supérieure à \SI{8}{\centi\meter}, ce qui correspond à une erreur relative d'environ \SI{7}{\percent}.}
	\label{fig_test_burdet_2000_failure}
\end{figure}
	%%
	%