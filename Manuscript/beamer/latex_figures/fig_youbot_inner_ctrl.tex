\begin{figure}
		\tikzsetnextfilename{fig_youbot_inner_ctrl}
		...
		\onslide<14,15>{
			...
			node[near end, right] {\tikzmark{pertInjectQd1}$\dot{\vect{q}}$\tikzmark{pertInjectQd2}};
			\draw[-{Latex[length=3mm]}] (c1.east) -- (cv.west);
		}
		\onslide<16->{
			\draw[{Latex[length=3mm]}-] (c2.north) --+ (0, 1) node[left]{$\vect{i}_p$};
		}
		
		\draw[-{Latex[length=3mm]}] (cv.east) -- (c2.west) node[above, yshift=2mm, xshift=-2mm]{$\vect{i}_d$};
