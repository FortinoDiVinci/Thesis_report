...
% equilibrium torque should compensate the effects of gravity
...
Cv = tf(zeros(3)); 
C  = tf(zeros(3));
C(2,2) = 1; % this matrix will be multiplied by the real corrector
for i = 1:3
    Cv(i,i) = Kv(i) + Kvi(i)/s;
end
...
Vsym = -(Fv + jacobian(C_3DOF(dth1,dth2,dth3,th2,th3)*[dth1;dth2;dth3], [dth1;dth2;dth3]));
...
Jdot = double(vpa(dJ0E_3DOF(th0(1),th0(2),th0(3)))); 

% see computation
for it = 1:5
    Ci = C*(K + Ki(it)/s);
    A{it} = J*iM*(J' - Cv*iJ*Ci);
...
for it = 1:5
    bodeplot(LD_Hz_rob{it}, str_col{it}, opts)
end
legend(num2str([Ki;Ki]))

%% Connection with an environment on the z axis

M_env = 1;
B_env = 10;
K_env = 100;

Hz_env = M_env*s^2 + B_env*s + K_env; % Fe/Z
%Hz_tot = (Hz_rob{3}/s)*