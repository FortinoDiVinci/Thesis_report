...
%     end
    ...
    rms_err_choice(i,1) = rms(nansum([ncycles(i).force(learn_idx); -b_cand(learn_idx)],1));
    ...
    if DO_SPLINE_INTERP && CONFIG_IDX == 1 % splines have only 1 config
        ...
        for i_pt = 1:nb_landings
            ...
            spline_pp = spline(ncycles(i).time(interp_indexes), ncycles(i).force(interp_indexes));
            ...
            spline_cand(i_pt, :) = ppval(spline_pp,ncycles(i).time(pred_indexes));
            spline_cost(i_pt) = rms(nansum([ncycles(i).force(pred_indexes); -spline_cand(i_pt, :)],1));
        end
        ...
        rms_err(i,3) = rms( ncycles(i).force(pred_indexes) - mean_spline_cand );
        rms_err(i,4) = rms( ncycles(i).force(pred_indexes) - b_spline_cand );
        rms_err(i,5) = rms( ncycles(i).force(pred_indexes) - w_spline_cand );
        
        errors(SIGNAL_IDX, CONFIG_IDX, 3).appendData(ncycles(i).force(pred_indexes) -...
                mean_spline_cand);
        errors(SIGNAL_IDX, CONFIG_IDX, 4).appendData(ncycles(i).force(pred_indexes) -...
             b_spline_cand);
...
%             b_spline_cand, 'Best Spline candidate');
        ...
        if DISP_SPLINE_ALL_CANDIDATES && i == 15%k+1
            ...
            hold on
            p0 = plot(ncycles(i).force, 'Color', [0 0.4470 0.7410]);
    ...
    rms_err(i, 1) = rms( ncycles(i).force(pred_indexes) - b_cand(pred_indexes) );
    rms_err(i, 2) = rms( ncycles(i).force(pred_indexes) - alt_b_cand(pred_indexes) );

    errors(SIGNAL_IDX, CONFIG_IDX, 1).appendData(ncycles(i).force([val_indexes,...
        pred_indexes]) - b_cand([val_indexes, pred_indexes]));
    errors(SIGNAL_IDX, CONFIG_IDX, 2).appendData(ncycles(i).force([val_indexes,...
        pred_indexes]) - alt_b_cand([val_indexes, pred_indexes]));
