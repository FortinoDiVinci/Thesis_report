classdef CYCLE_DATA < handle
    ...
    methods
        ...
        function self = setPerturbation(self, perturbation_value, perturbation_time)
            if any(perturbation_value ~= 0) % TODO: improve multiple perturbation managment
                self.isPerturbed = 1;
                self.pert_val = perturbation_value(1);
                self.pert_dir = sign(self.pert_val);
                self.pert_t = perturbation_time(1);
                self.pert_idx = find(self.pert_t <= self.time, 1, 'first');
                self.pert_idx_global = self.idx(1) + self.pert_idx - 1;
            end
        end
        %
