...
%initial parameters
param_init(1) = 1;    % A(1)
param_init(2) = 0.1;  % A(2)
param_init(3) = 0.1;  % A(3)
param_init(4) = 1;    % f(1)
param_init(5) = 2;    % f(2)
param_init(6) = 5.5;  % f(3)
param_init(7) = pi/2; % phi(1)
param_init(8) = 0;    % phi(2)
...
idx_fit_i = [idx_p_i(1)+(-fit_l:1:-1), idx_p_i(end)+(1:1:fit_l)];
t_idx = t(idx_fit_i);
...
param_opt.A(2) = p_argmin(2);
param_opt.A(3) = p_argmin(3);
param_opt.f(1) = p_argmin(4);
param_opt.f(2) = p_argmin(5);
param_opt.f(3) = p_argmin(6);
param_opt.phi(1) = p_argmin(7);
param_opt.phi(2) = p_argmin(8);
