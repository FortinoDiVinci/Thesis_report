...
% a human arm rhythmic trajectory (complete cycles are used here)
...
% complete cycles
t_cycle = cell(1, length(idx_peaks)-2); 
z_cycle = cell(size(t_cycle));
fz_cycle = cell(size(t_cycle));
half_cycle = nan(size(t_cycle)); % middle peak position on the cycle
% normalized cycles
zn_cycle = cell(size(t_cycle));
fzn_cycle = cell(size(t_cycle));

% half cycle selection and normalization 
for pk_idx = 2:length(idx_peaks)-1
    idx_cycle = idx_peaks(pk_idx-1):idx_peaks(pk_idx+1)+idx_traj_fit;
    half_cycle(pk_idx-1) = length(idx_peaks(pk_idx-1):idx_peaks(pk_idx));
    t_cycle{pk_idx-1} = t(idx_cycle);
    z_cycle{pk_idx-1} = z(idx_cycle);
    fz_cycle{pk_idx-1} = fz(idx_cycle);
    %normalization: signal/amp(signal) - min(signal/amp(signal))
    zn_cycle{pk_idx-1} = z_cycle{pk_idx-1}/(max(z_cycle{pk_idx-1}) - min(z_cycle{pk_idx-1}));
    zn_cycle{pk_idx-1} = zn_cycle{pk_idx-1} - min(zn_cycle{pk_idx-1});
    fzn_cycle{pk_idx-1} = fz_cycle{pk_idx-1}/(max(fz_cycle{pk_idx-1}) - min(fz_cycle{pk_idx-1}));
    fzn_cycle{pk_idx-1} = fzn_cycle{pk_idx-1} - min(fzn_cycle{pk_idx-1});
end
...
% sort normalized cycles according to the presence of perturbation 
for cyc_idx = 1:length(zn_cycle)
    if ~any( (min(t_cycle{cyc_idx}) <= t_on_dist) & ...
        ...
        half_cycle_np(cyc_np_idx) = half_cycle(cyc_idx);
    ...
    % rising or decreasing phase for the second half of the cycle
    if zn_cycle{cyc_idx}(half_cycle(cyc_idx)) < zn_cycle{cyc_idx}(end-idx_traj_fit)
...
% middle peak position of the norm. cycle
halfn_cycle_np = half_cycle_np .* length(zn2_cycle_np(:,1)) ./ cellfun('length', zn_cycle_np);
...
for cycle_idx = 1:length(validation_cycles(1,:))
    fit_err = validation_cycles(idx_fit(cycle_idx,:),cycle_idx) - learning_cycles(idx_fit(cycle_idx,:),:);
    fit_err_avg = mean(fit_err);
    fit_err_std = std(fit_err);
    fit_err_score = abs(fit_err_avg) + fit_err_std;
    [~,min_score_idx] = min(abs(fit_err_score));

    val_cycle_idx = floor(length(zn2_cycle_np(1,:))/2)+cycle_idx;
    cycle = zn_cycle_np{val_cycle_idx};
